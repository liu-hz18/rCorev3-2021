...
fn main() -> i32 {
    ...
    if pid == 0 {
        ...
        assert_eq!(mail_read(&mut buffer), -1); // 读邮箱，邮箱空
        ...
        assert_eq!(mail_read(&mut buffer), -1); // 邮箱空
        println!("child read 1 mail fail");
        println!("child sleep 1s");
        sleep(1500 as usize);
        assert_eq!(mail_read(&mut buffer), BUF_LEN as isize); // 此时邮箱又可读了
    ...
    for i in 0..16 {
        ...
        assert_eq!(mail_write(pid as usize, &buffer), BUF_LEN as isize); // 向子进程邮箱写入
    }
    println!("father wirte 16 mails succeed");
    let buffer = [16 as u8; BUF_LEN];
    assert_eq!(mail_write(pid as usize, &buffer), -1); // 邮箱满
    println!("father wirte 1 mail fail");
    println!("father sleep 2s");
    sleep(2000 as usize); // 此时子进程已经读完，邮箱空
    assert_eq!(mail_write(pid as usize, &buffer), BUF_LEN as isize); // 再写入一个邮箱
    println!("father wirte 1 mail succeed");

    let mut xstate: i32 = -100;
    assert!(wait(&mut xstate) > 0); // 等待子进程结束
    assert_eq!(xstate, 0);
